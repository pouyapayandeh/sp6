...
class Pnet5():
    ...
    def activation(self,data_in):
        return keras.layers.ReLU()(data_in)
    ...
    def get_optimizer(self):
        ...
        return keras.optimizers.Adam(learning_rate=self.config['optimizer']['lr'], decay=self.config['optimizer']['decay'],
                                     epsilon=self.config['optimizer']['epsilon'],amsgrad = True,clipvalue = 1000)
    ...
    def get_callbacks(self):
        ...
        return [
            ...
            LRTensorBoard(log_dir="log/"+self.config['training']['path'], histogram_freq=1,write_images=True,profile_batch=(10,15)),
        ]

    def count_params(self, objs):
        return np.sum([keras.backend.count_params(w) for w in objs])
    ...
    def fit_model(self, train_set_generator, num_train_samples, test_set_generator, num_test_samples, num_epochs):
        ...
        print ('Total params: %d'%(self.count_params(self.model.weights)))
        print ('Trainable params: %d'%(self.count_params(self.model.trainable_weights)))
        print ('Non Trainable params: %d'%(self.count_params(self.model.non_trainable_weights)))
    ...
    def build_model(self):
        ...
        data_out_speech = keras.layers.Lambda(lambda x: keras.backend.squeeze(x, -1),
                                              output_shape=lambda shape: (shape[0], shape[1]), name='data_output_1')(
            data_out_speech)
        data_out_noise = keras.layers.Lambda(lambda x: keras.backend.squeeze(x, -1),
    ...
    def fft_layer(self,data_x):
        ...
        data_x_squeez = keras.layers.Lambda(lambda x: keras.backend.squeeze(x, 2),
                                              output_shape=lambda shape: (shape[0], shape[1]), name='data_x_squeez')(data_x_padded)

        data_x_rfft = keras.layers.Lambda(lambda x: tf.signal.rfft(x),name = 'data_x_rfft')(data_x_squeez)

        data_x_rfft_abs = keras.layers.Lambda(lambda x: tf.math.abs(x) , name='data_x_rfft_abs')(data_x_rfft)
        data_x_rfft_angle_complex = keras.layers.Lambda(lambda x: tf.math.exp(tf.complex(0.0,tf.math.angle(x))) ,
                                                             name='data_x_rfft_angle_complex')(data_x_rfft)
        # Learning Filters 800*100 100
        rfft_process = data_x_rfft_abs
        rfft_process_dense = keras.layers.Dense(units=filter_num,activation="sigmoid",
                        use_bias=False,kernel_initializer=keras.initializers.Identity())(rfft_process)
        # rfft_process_dense = layers.AddSingletonDepth()(rfft_process_dense)
        rfft_process_dense = keras.layers.RepeatVector(rfft_process.shape[-1])(rfft_process_dense)


        rfft_process = layers.AddSingletonDepth()(data_x_rfft_abs)
        rfft_process_filter = layers.BigFilter(units=filter_num,name="freq_filters")(rfft_process)

        rfft_process = keras.layers.Multiply(name='rfft_process')([rfft_process_dense,rfft_process_filter])
        rfft_process = keras.layers.Convolution1D(1, 1)(rfft_process)
        # rfft_process = self.activation(rfft_process)
        #Recontruction
        rfft_process_complex = keras.layers.Lambda(lambda x: tf.complex(tf.math.abs(x),0.0) ,
                                                             name='rfft_process_complex')(rfft_process)
        new_rfft= keras.layers.Multiply(name='new_rfft')([layers.AddSingletonDepth()(data_x_rfft_angle_complex) ,
                                                        rfft_process_complex])
        
        new_rfft = keras.layers.Lambda(lambda x: keras.backend.squeeze(x, -1))(new_rfft)
        data_out= keras.layers.Lambda(lambda x: tf.signal.irfft(x),name = 'IRFFT')(new_rfft)
        data_out = keras.layers.Lambda(lambda x: keras.backend.expand_dims(x, -1))(data_out)
        data_out = tf.keras.layers.Cropping1D(cropping=(0, 1),name="data_fft_put")(data_out)
        return data_out
