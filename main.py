...
def training(config, cla):
    ...
    model = models.Pnet5(config, load_checkpoint=cla.load_checkpoint, print_model_summary=cla.print_model_summary)
...
def get_valid_output_folder_path(outputs_folder_path,text = "samples_%d"):
    j = 1
    while True:
        output_folder_name = text % j
...
def inference(config, cla):
    ...
    if not bool(cla.one_shot):
        model = models.Pnet5(config, target_field_length=cla.target_field_length,
    ...
    for filename in filenames:
        ...
        if bool(cla.one_shot):
            ...
            model = models.Pnet5(config, load_checkpoint=cla.load_checkpoint, input_length=len(input['noisy']), print_model_summary=cla.print_model_summary)
...
def explain(config, cla):
    ...
    samples_folder_path = os.path.join(config['training']['path'], 'explain')
    output_folder_path = get_valid_output_folder_path(samples_folder_path,"explain_%d")
    ...
    for filename in filenames:
        ...
        print ("Explaining: " + filename)
        explain_util.explain(model, input, output_filename_prefix,
                                            config['dataset']['sample_rate'], output_folder_path)
        break
